...
function Hero() { return <section className="hero" id="top"><div className="hero-grid"/><div className="hero-portrait" aria-hidden="true"><img src={profileImage} alt=""/></div><div className="eyebrow"><span>MACHINE LEARNING ENGINEER</span><span>SEATTLE, WA</span></div><h1 className="dynamic-title"><Typewriter/></h1><div className="hero-foot"><div><p>Hi, I’m Manu. I’m a Machine Learning Engineer at <strong>Qualcomm</strong>, working on on-device ML. My background is in computer vision research, and I enjoy turning research ideas into efficient systems that work in the real world.</p><div className="hero-focus"><span>FOCUS</span>{['On-Device ML','Computer Vision','Efficient Inference','LLM Systems'].map(x => <b key={x}>{x}</b>)}</div></div><a href="#about" className="round-link" aria-label="Read about Manu"><Arrow down/></a></div><div className="coordinate">47.6062° N<br/>122.3321° W</div><div className="hero-socials hero-side-socials">{socials.map(([name,url]) => <a key={name} href={url} target="_blank" rel="noreferrer"><SocialIcon name={name}/>{name}</a>)}</div></section> }
